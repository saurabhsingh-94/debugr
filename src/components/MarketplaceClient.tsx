...
export default function MarketplaceClient({ initialPrompts, user }: { initialPrompts: any[], user: any }) {
  ...
  const filteredPrompts = initialPrompts.filter((p) => {
    ...
    const descMatch = (p.description ?? p.previewContent ?? "").toLowerCase().includes(searchQuery.toLowerCase());
    const matchesSearch = titleMatch || descMatch;
    const matchesChannel =
      activeChannel === "All Intelligence" ||
      (p.category ?? "").toLowerCase() === activeChannel.toLowerCase();
          ...
          <div className="flex items-center gap-10 overflow-x-auto no-scrollbar py-4 border-y border-white/[0.03]">
            {channels.map((channel) => (
              <button
                ...
                className={`whitespace-nowrap text-[10px] font-bold uppercase tracking-[0.3em] transition-all relative ${
                  activeChannel === channel ? "text-white" : "text-zinc-700 hover:text-zinc-400"
                }`}
              >
                {channel}
                {activeChannel === channel && (
                  <motion.div
                    layoutId="market-line"
                    className="absolute -bottom-4 left-0 right-0 h-[1px] bg-white"
                  />
                )}
              </button>
            ))}
      ...
      {filteredPrompts.length === 0 ? (
        <div className="py-32 text-center border-2 border-dashed border-white/5 rounded-[48px]">
          ...
          <p className="text-zinc-600 text-[10px] font-bold uppercase tracking-[0.2em] mt-2">
            Try adjusting your spectral filters
          </p>
        </div>
      ) : (
