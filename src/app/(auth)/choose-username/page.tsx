...
export default function ChooseUsernamePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!username || username.length < 3) {
      toast.error("Invalid Username: Name is too short");
      return;
    }

    if (isAvailable === false) {
      toast.error("Username Taken: Identity collision");
      return;
    }
    ...
    try {
      ...
      toast.success("Identity Verified: Link successful");
           ...
           <div className="space-y-12 relative">
             <div className="text-center space-y-4">
               <div className="w-16 h-16 bg-white/[0.02] border border-white/10 rounded-3xl flex items-center justify-center mx-auto mb-8 shadow-2xl">
                ...
                <h1 className="text-3xl font-black italic tracking-tighter text-white uppercase">Define Identity</h1>
                <p className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.5em] mt-4">Choose a unique username to establish presence</p>
             </div>
             ...
             <div className="p-6 bg-white/[0.01] border border-white/5 rounded-3xl flex items-start gap-4">
               <ShieldCheck className="w-4 h-4 text-zinc-600 mt-1" />
                <p className="text-[9px] font-bold text-zinc-700 uppercase leading-relaxed tracking-wider">
                  Your identity is unique to this network. Once verified, it becomes your unique identifier.
                </p>
             </div>
           </div>
        </div>
